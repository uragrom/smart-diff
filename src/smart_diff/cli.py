...
from .output import print_analysis, print_commit_message, print_error, print_info
...
@click.argument("key", type=click.Choice(["model", "lang"]))
@click.argument("value", required=True)
def config_set(key: str, value: str) -> None:
    """Set default model or language. E.g. smart-diff config set model deepseek-r1."""
...
def config_show() -> None:
    """Show current default model and language."""
    cfg = load_config()
...
def run(
    ...
    if not diff.strip():
        print_error(t("no_changes", lang if lang != "auto" else "en"), lang if lang != "auto" else "en")
        sys.exit(1)

    if analyzing_last:
        print_info(t("analyzing_last", lang if lang != "auto" else "en"), lang if lang != "auto" else "en")

    if commit_msg:
        print_info(t("commit_msg_generating", lang if lang != "auto" else "en", model=model), lang if lang != "auto" else "en")
        try:
            msg = generate_commit_message(diff, model, llm_lang)
            if output_file:
                output_file.write_text(msg, encoding="utf-8")
                print_info(t("commit_written", lang if lang != "auto" else "en", path=str(output_file)), lang if lang != "auto" else "en")
            else:
    ...
    print_info(t("model_label", lang if lang != "auto" else "en", model=model), lang if lang != "auto" else "en")
    try:
